...
import { fetchEssayParagraph, fetchRandomWords, generateWordsFromList } from './modules/api';
...
async function loadNewContent() {
    if (currentMode === 'essay') {
        ...
        if (elements.essayDisplay) {
            elements.essayDisplay.innerHTML = '<span class="essay-char pending">Loading essay...</span>';
        }
...
function renderEssay() {
    ...
    elements.essayDisplay.innerHTML = html || '<span class="essay-char pending">Loading essay...</span>';
}
...
function updateEssayState() {
    ...
    const progressRatio = currentEssay.length ? typed.length / currentEssay.length : 0;
    ...
    gameState.wpm = calculateWPM(startTime, Math.max(1, Math.floor(typed.length / 5)));

    updateStatsDisplay(gameState.score, gameState.health, gameState.wpm, elements.scoreEl, elements.healthEl, elements.wpmEl);
...
function resetGameLogic() {
    ...
    wordsTyped = 0;
    resetState();
    updateStatsDisplay(0, 100, 0, elements.scoreEl, elements.healthEl, elements.wpmEl);
}

function showGameCompleteModal() {

    if (elements.title) elements.title.textContent = 'Essay completed';
    if (elements.finalScoreEl) elements.finalScoreEl.textContent = gameState.score;
...
async function resetGame() {
    ...
    updateStatsDisplay(gameState.score, gameState.health, 0, elements.scoreEl, elements.healthEl, elements.wpmEl);
...
function updateProgressBar() {
    if (currentMode === 'essay') {
        const ratio = currentEssay.length ? elements.userInput.value.length / currentEssay.length : 0;
